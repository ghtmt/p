...
class Spider(Spider):
    ...
    def init(self, extend=''):
        ...
        if extend and extend.strip():
            try:
                config = json.loads(extend.strip())
            except:
                config = {}
        
        host = config.get('host', '').rstrip('/')
        if not host or not re.match(r'^https?://[a-zA-Z0-9-]+(\.[a-zA-Z0-9-]+)*(:\d+)?/?$', host): 
            return None
            
        appid = config.get('app_id', '').rstrip('/')
        deviceid = config.get('deviceid', '').rstrip('/')
        version_code = config.get('versionCode', '').rstrip('/')
        channel = config.get('UMENG_CHANNEL', '').rstrip('/')
        
        if not deviceid or not self.is_valid_android_id(deviceid):
            deviceid = self.generate_android_id()
            
        if not(appid and deviceid and version_code and channel): 
            return None
